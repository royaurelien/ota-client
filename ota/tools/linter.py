...
def build_messages_modules(messages):
    ...
    for module, module_messages in data.items():
        yield (module, sorted(module_messages, key=lambda x: x.get("line")))
...
class Report:
    ...
    def __init__(self, messages, stats=None, previous_stats=None, template=None):
        ...
        self.modules = sorted(self._module_messages.items(), key=lambda x: x[0].path)
    ...
    def render(self):
        ...
        return template.render(
            messages=self._messages, metrics=self.metrics, report=self
        )
...
def build_command_parser():
    ...
    parser = argparse.ArgumentParser(description="Transform Pylint JSON report to HTML")
    parser.add_argument(
        "filename",
        metavar="FILENAME",
    ...
    parser.add_argument(
        ...
        action="store",
        default="utf-8",
        help="Encoding used to write output file (if not stdout); " "default to utf-8",
    )
    parser.add_argument(
        "-t",
        "--template",
